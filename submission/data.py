...
class Data:
    ...
    def fillna_neighbor(self, train_df, df, col, k):
    	model = KNeighborsClassifier(n_neighbors=k)
    	train = train_df.loc[~train_df[col].isnull(), ['latitude', 'longitude', col]]
	...
	self.train = self.data[~self.data[self.target_name].isnull()]
	...
	for col in fill_neighbor:
		if self.data[col].isnull().sum() > 0:
			self.fillna_neighbor(self.train, self.data, col, 5)
